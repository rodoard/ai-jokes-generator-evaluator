...
export const actions = {
  generate: async ({ request }) => {
    ...
    const temperature = data.get("temperature") / 10
    const prompt = `
    Tell me a joke about ${topic} in a ${tone}, a ${kind} of joke.
    `
    const { text, finishReason } = await experimental_generateText({
      model: AI_MODEL,
      max_tokens: 512,
      system,
      prompt: `Tell me a joke about ${topic} in a ${tone}, a ${kind} of joke.`,
      temperature,
    });
    return {
      prompt,
